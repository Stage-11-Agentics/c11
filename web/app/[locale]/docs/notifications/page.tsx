...
echo "$CMUX_NOTIFICATION_TITLE: $CMUX_NOTIFICATION_BODY" >> ~/notifications.log`}</CodeBlock>
      ...
      <h3>{t("cli")}</h3>
      <CodeBlock lang="bash">{`c11 notify --title "Task Complete" --body "Your build finished"
c11 notify --title "Claude Code" --subtitle "Waiting" --body "Agent needs input"`}</CodeBlock>
...
case "$EVENT_TYPE" in
    "Stop")
        c11 notify --title "Claude Code" --body "Session complete"
        ;;
    "PostToolUse")
        [ "$TOOL" = "Task" ] && c11 notify --title "Claude Code" --body "Agent finished"
        ;;
esac`}</CodeBlock>
      <CodeBlock lang="bash">{`chmod +x ~/.claude/hooks/c11-notify.sh`}</CodeBlock>
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/c11-notify.sh"
          }
        ]
      }
    ],
...
notify-after() {
  ...
  if [ $exit_code -eq 0 ]; then
    c11 notify --title "✓ Command Complete" --body "$1"
  else
    c11 notify --title "✗ Command Failed" --body "$1 (exit $exit_code)"
  fi
  return $exit_code
}
