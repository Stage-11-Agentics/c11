...
> Note: 0.62.x and 0.63.0 were never published. If you are coming from 0.61.0, this release carries everything since.

### Added

- **The A button is now a split agent launcher.** Click the main zone to launch the effective default, or use press-and-hold, right-click, or the new caret to open a keyboard-ready picker for your saved configurations and launch stats. The picker stays anchored to the button, scrolls long shortlists, explains unavailable harnesses inline, and keeps digit launch shortcuts aligned. ([#355](https://github.com/Stage-11-Agentics/c11/pull/355), [#357](https://github.com/Stage-11-Agentics/c11/pull/357), [#395](https://github.com/Stage-11-Agentics/c11/pull/395), [#396](https://github.com/Stage-11-Agentics/c11/pull/396), [#397](https://github.com/Stage-11-Agentics/c11/pull/397), [#412](https://github.com/Stage-11-Agentics/c11/pull/412)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
...
- **The launch picker is three things, and the config editor reads Provider → Model → Effort → Harness.** The picker is a header, your saved agent rows, and one `Edit Launch Agents` footer; the RECENT section and follow-recent mode are gone, along with their on-disk schema. Recent launches keep being recorded as telemetry rather than feeding resolution. In the editor, Provider is now a real axis instead of a label derived from harness, and harness comes last, filtered to what the chosen model supports. ([#412](https://github.com/Stage-11-Agentics/c11/pull/412)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **c11 sends far less crash telemetry, and groups what it does send by the right stack.** The Sentry client now holds a client-side daily event budget with a separate sub-cap on hang reports, so one wedged install can no longer drain the allowance for everybody, and the CLI's long-dead DSN is gone. Main-thread hang reports fingerprint on the wedged stack rather than the watchdog's, which splits what had collapsed into a single issue back into the distinct hangs behind it. ([#401](https://github.com/Stage-11-Agentics/c11/pull/401), [#402](https://github.com/Stage-11-Agentics/c11/pull/402), [#404](https://github.com/Stage-11-Agentics/c11/pull/404)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **A slow heartbeat behind an unwatched window is no longer reported as a hang.** Main parked at the top of an idle run loop is an app ready to handle your next click, not a freeze — and off-screen it is App Nap and timer coalescing working as designed. That pattern was 46.5% of all captured hang episodes and none of it was a freeze anyone could see; it now reports only when a window is actually in front of you. What does get reported carries the frames that name the wedge. ([#411](https://github.com/Stage-11-Agentics/c11/pull/411), [#413](https://github.com/Stage-11-Agentics/c11/pull/413)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!

### Fixed

- **The `c11` CLI no longer crashes when whatever was reading its output goes away.** Writing to a closed pipe raised an Objective-C exception, and because such an exception cannot unwind through Swift frames it went straight to `abort()` — so a CLI invocation whose terminal or agent had just exited died with SIGABRT and a crash report instead of finishing quietly. Every CLI write now goes through a `write(2)` path where a vanished reader is an ordinary return value. Most visible when c11 itself is shutting down, which is exactly when in-flight commands lose their pipes. ([#419](https://github.com/Stage-11-Agentics/c11/pull/419)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **The A button was dead, and now every refusal to launch says why.** A pinned default that had drifted to a custom harness with no command resolved to an empty command string, and the launch path returned silently — no surface, no error, nothing. Both halves are closed: the read side heals a drifted factory seed and refuses to pin an unlaunchable config, the write side refuses to save one, and any decline now writes its reason into the picker's notice bar instead of vanishing. A bare A-click that cannot launch *opens* the picker with the reason pre-seeded, landing you where you can fix it. ([#412](https://github.com/Stage-11-Agentics/c11/pull/412)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **A browser prompt can no longer freeze the entire app.** Four browser paths — the `http://` navigation warning, JavaScript alert/confirm/prompt, and their popup-window twins — fell back to an app-modal alert whenever no window was available to sheet onto. Since browser surfaces are routinely driven over the socket with the window backgrounded, that meant a nested run loop on the main thread holding every terminal, pane, and agent hostage behind a dialog nobody could see: measured at a median of 8.5 minutes and a maximum of 6.8 hours. Sheet hosting now searches all app windows, and when there genuinely is no window to ask on, the decision resolves with its safe default (navigation denied, nothing added to the allowlist) instead of prompting. ([#417](https://github.com/Stage-11-Agentics/c11/pull/417)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **Menu-bar visibility changes no longer stall the app on LaunchServices.** Reading the app's activation policy on every `UserDefaults` change forwarded into a synchronous LaunchServices XPC round-trip from the main thread, with nothing bounding the wait — observed stalling for 23 seconds. ([#414](https://github.com/Stage-11-Agentics/c11/pull/414)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
...
- **A Codex sub-agent finishing no longer pulls its parent surface into Waiting.** Codex's legacy `notify` callback fires for child threads as well as the root one, and c11 had no way to tell them apart. The callback payload now rides along with the notification, and c11 delivers it only when the payload's `thread-id` matches the thread it captured for that surface; anything else is dropped. ([#399](https://github.com/Stage-11-Agentics/c11/pull/399)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
...
- **Automatic updates work again.** The GhosttyKit build workflow published its xcframework tarball as a normal release, so the newest one captured GitHub's "latest" pointer. Sparkle's feed resolves through `releases/latest/download/appcast.xml`, which then pointed at a release carrying no appcast, and every shipped c11 failed its update check with `SUDownloadError 2001`. Artifact releases are now created as prereleases with `--latest=false`, and the three existing ones were corrected. ([#406](https://github.com/Stage-11-Agentics/c11/pull/406)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **A display change can no longer wedge the app permanently.** `ghostty_surface_set_display_id` pushed `.forever` into a 64-slot renderer mailbox from the main thread, so once that mailbox filled the main thread blocked with no timeout and never recovered: force-quit was the only exit. Observed in the wild as a single process wedged for nearly fourteen hours. ([#403](https://github.com/Stage-11-Agentics/c11/pull/403)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
- **Agent launches now keep working-directory intent and project configuration aligned.** `launch-agent` resolves explicit cwd, workspace root, then launching surface; linked worktrees proceed with a named path-bearing warning on stderr and in JSON. Project `.c11/agents.json` lookup follows that same resolved cwd instead of the GUI app process, reports its matched `config_source`, and the legacy `default-agent launch --in-surface` rail follows the target surface too. ([#407](https://github.com/Stage-11-Agentics/c11/pull/407)) — thanks [@BenevolentFutures](https://github.com/BenevolentFutures)!
