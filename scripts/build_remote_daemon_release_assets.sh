...
Builds c11d-remote release assets for the supported remote platforms and emits:
  c11d-remote-<goos>-<goarch>
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build c11d-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/c11d-remote-* "$OUTPUT_DIR"/c11d-remote-checksums.txt "$OUTPUT_DIR"/c11d-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="c11d-remote-checksums.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/c11d-remote-manifest.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/c11d-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="c11d-remote-${GOOS}-${GOARCH}"
...
echo "Built c11d-remote assets in ${OUTPUT_DIR}"
