...
struct GhosttyConfig {
    ...
    private static let cmuxReleaseBundleIdentifier = "com.stage11.c11"
    ...
    private static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        if let bundleDefaults = readBundledC11muxDefaults() {
            config.parse(bundleDefaults)
        }
...
extension NSColor {
    convenience init?(hex: String) {
        ...
        guard (hexSanitized.count == 6 || hexSanitized.count == 8),
              let raw = UInt64(hexSanitized, radix: 16) else {
            return nil
        }
        ...
        if hexSanitized.count == 6 {
            ...
            r = CGFloat((raw & 0xFF000000) >> 24) / 255.0
            g = CGFloat((raw & 0x00FF0000) >> 16) / 255.0
            b = CGFloat((raw & 0x0000FF00) >> 8) / 255.0
            a = CGFloat(raw & 0x000000FF) / 255.0
        }
