...
- `DONE` local app probes remote platform, verifies a release-pinned `c11d-remote` artifact by embedded manifest SHA-256, uploads it when missing, and runs `serve --stdio`.
...
1. `DONE` `c11d-remote` now supports proxy stream RPC (`proxy.open`, `proxy.close`, `proxy.write`, `proxy.stream.subscribe`) with pushed `proxy.stream.data/eof/error` events.
